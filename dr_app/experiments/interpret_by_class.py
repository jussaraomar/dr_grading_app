...
# from utils.interpretability import ViTInterpretability
# from utils.grad_cam import ResNetGradCAM, EfficientNetGradCAM
# from config.paths import paths


# def create_retinal_mask_from_image(original_image):
#     """
#     Create a binary mask of the retinal area (non-black pixels).
#     This matches what we do in GradCAM.visualize_cam.
#     """
...
#         resnet.load_state_dict(torch.load(f"{paths.saved_models}/resnet_finetuned_best.pth", weights_only=True))
#         resnet.eval()
...
#         efficientnet.load_state_dict(torch.load(f"{paths.saved_models}/efficientnet_finetuned_best.pth", weights_only=True))
#         efficientnet.eval()
...
#         vit.load_state_dict(torch.load(f"{paths.saved_models}/vit_finetuned_best.pth", weights_only=True))
#         vit.eval()
#         models['vit'] = vit
...
#             print(f"✅ Class {class_idx}: Randomly selected {samples_per_class} samples")
#         else:
#             class_samples[class_idx] = samples
#             print(f" Class {class_idx}: Only {len(samples)} samples available (requested {samples_per_class})")
    
#     return class_samples

# def interpret_class_samples(models, class_samples, outputs_dir, device):
#     """Generate interpretability for all models"""
#     all_results = {}
...
#             # Get original image
#             original_img_path = os.path.join(paths.images_dir, sample_data['image_id'] + '.png')
...
# def interpret_single_sample(models, image_tensor, original_image, true_class, sample_idx, outputs_dir, device, image_id=None):
#     """Generate interpretability visualizations for all models on one sample"""
...
#             save_path = os.path.join(sample_dir, f"resnet_gradcam.png")
#             result = resnet_cam.visualize_cam(image_tensor, original_image, true_class=true_class, class_idx=None, save_path=save_path)
...
#             save_path = os.path.join(sample_dir, f"efficientnet_gradcam.png")
#             result = efficientnet_cam.visualize_cam(image_tensor, original_image, true_class=true_class, class_idx=None, save_path=save_path)
...
#     if 'vit' in models:
#         try:
#             vit_interpreter = ViTInterpretability(models['vit'], device)
#             save_path = os.path.join(sample_dir, f"vit_interpretability.png")
#             result = vit_interpreter.visualize_interpretation(image_tensor, original_image, 'grad_cam', true_class, save_path)
#             results['vit'] = result
#             print("    ViT interpretability completed")
#         except Exception as e:
#             print(f"    ViT interpretability failed: {e}")
    
#     # Create comparison figure if we have results
#     if results:
...
#     """comparison figure showing all model interpretations"""
#     if len(results) < 2:
#         return
...
#     """Create a summary of results for each class"""
...
#                         'pred_class': result['pred_class'],
#                         'confidence': result['confidence'],
#                         'correct': result.get('true_class', class_idx) == result['pred_class']
#                     })
                    
#                     status = " CORRECT" if result.get('true_class', class_idx) == result['pred_class'] else " WRONG"
#                     print(f"  Sample {sample_idx} - {model_name.upper()}: {status} (True: {result.get('true_class', class_idx)}, Pred: {result['pred_class']}, Conf: {result['confidence']:.3f})")
...
#         print(f"{model_name.upper()}: {accuracy:.1f}% accuracy ({total_samples} samples)")
...
#             print(f"Class {class_idx} - {model_name.upper()}: {correct}/{total} correct ({accuracy:.1f}%)")
...
#     train_df, temp_df = train_test_split(full_df, test_size=0.3, random_state=42, stratify=full_df['diagnosis'])
...
#     all_results = interpret_class_samples(models, class_samples, outputs_dir, device)
...
#     print(f" Analyzed {samples_per_class} samples from each of {len(class_samples)} classes")
...
def create_retinal_mask_from_image(original_image):
    """
    Create a binary mask of the retinal area (non-black pixels).
    Used to suppress the black background so maps look cleaner.
    """
    original_np = np.array(original_image)  # [H, W, 3]
    gray = cv2.cvtColor(original_np, cv2.COLOR_RGB2GRAY)
    mask = gray > 5
